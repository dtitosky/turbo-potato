    ...
    try:
        ...
        response = openai.Client().chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """Ты опытный диетолог. Составь персонализированные рекомендации по питанию 
                    на основе анализов крови. Используй эмодзи и разметку для лучшей читаемости.
                    
                    Формат ответа (строго следуй этой структуре):
                    
                    🥗 *РЕКОМЕНДУЕМЫЕ ПРОДУКТЫ:*
                    (используй эмодзи для каждого пункта)
                    • продукт/группа продуктов - краткое обоснование
                    
                    ⛔️ *ПРОДУКТЫ К ОГРАНИЧЕНИЮ:*
                    (используй эмодзи для каждого пункта)
                    • продукт/группа продуктов - причина ограничения
                    
                    ⏰ *РЕЖИМ ПИТАНИЯ:*
                    (используй эмодзи для каждой рекомендации)
                    • конкретная рекомендация по режиму
                    
                    💡 *ВАЖНЫЕ ПРИМЕЧАНИЯ:*
                    • 2-3 ключевых момента
                    
                    Используй соответствующие эмодзи для продуктов (🥩🥑🥕🍎), 
                    действий (🔄⬆️⬇️) и состояний (✅⚠️).
                    Используй маркированные списки и жирный шрифт для заголовков."""
                },
                {
                    "role": "user",
                    "content": f"""На основе этих результатов анализов крови составь структурированные 
                    рекомендации по питанию с эмодзи:
        ...
        # Добавляем предупреждение в конце
        recommendations = response.choices[0].message.content
        return recommendations + "\n\n⚠️ _Данные рекомендации носят общий характер. Обязательно проконсультируйтесь с вашим лечащим врачом._"
    except Exception as e:
