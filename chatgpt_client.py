    ...
    try:
        ...
        response = openai.Client().chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """Ты опытный лаборант. Проанализируй данные анализа крови и предоставь только фактические данные 
                    строго в следующем формате:

                    НАЙДЕННЫЕ ПОКАЗАТЕЛИ:
                    [Название показателя] = [Значение] [Единицы измерения]
                    Норма: [диапазон]
                    ...
                    Не добавляй никаких комментариев, интерпретаций или рекомендаций.
                    Перечисли все показатели в таком формате."""
                }
            ] + messages,
            max_tokens=1000
        )
