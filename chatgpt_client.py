            ...
            for page_num, img in enumerate(images_base64):
                response = openai.Client().chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": """ВАЖНО: Ты должен строго следовать этой инструкции без отклонений.
                            
                            1. Найди в изображении все показатели анализа крови
                            2. Для каждого показателя выведи ровно три строки в формате:
                            
                            [Показатель] = [Число] [Ед.изм.]
                            Норма: [мин]-[макс] [Ед.изм.]
                            Статус: [НОРМА/ВЫШЕ/НИЖЕ]
                            
                            3. Между показателями оставляй одну пустую строку
                            4. НЕ ДОБАВЛЯЙ никаких других строк, комментариев или текста
                            5. НЕ ПИШИ заголовков или вводного текста
                            6. Просто начни сразу с первого показателя"""
                        },
                        {
                            "role": "user",
                            "content": [
                                {
                                    "type": "text",
                                    "text": f"Это страница {page_num + 1} из PDF. Перечисли все показатели анализа крови из изображения строго в требуемом формате."
                                },
                                {
                                    "type": "image_url",
                                    "image_url": {
