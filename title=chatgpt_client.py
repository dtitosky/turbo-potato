...
+{analysis_text}
+
+Дай подробные рекомендации по дальнейшим действиям, питанию и образу жизни. Ответ сформируй в формате Markdown, строго соблюдая следующий формат:
+  - Каждый пункт списка должен начинаться с эмодзи, затем пробел, затем текст рекомендации.
+  - Если рекомендация относится к питанию, используй, например:
+       • 🍏 — для рекомендаций по фруктам,
+       • 🥦 — для рекомендаций по овощам,
+       • 🍽️ — для рекомендаций по режиму приема пищи,
+       • 💧 — для рекомендаций по гидратации.
+Не добавляй никаких дополнительных комментариев или лишнего текста.
+"""
